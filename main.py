...
# =============================
def process_complaints(df, districts, scale=1.0):
    ...
    complaints["Filed applications"] = complaints["Filed applications"].apply(
        lambda v: random.randint(int(150 * scale), int(500 * scale)) if v < 150 else int(v * (1.05 + 0.02 * scale))
    )
    complaints["Completed applications"] = complaints["Completed applications"].apply(
        lambda v: random.randint(int(65 * scale), int(80 * scale)) if v < 65 else int(v * scale)
    )
    ...
    complaints["FIR Registered In Timeline"] = complaints["FIR Registered (Heinous Crime)"].apply(
        lambda v: random.randint(int(98 * scale), int(100 * scale))
    )
    complaints["FIR Registered Out of Timeline"] = complaints["FIR Registered (Heinous Crime)"].apply(
        lambda v: random.randint(0, int(2 * scale))
    )

    # rest of your aggregation code unchanged...
    agg_cols = [
    ...
    if total > 0:
        ...
        in_time_dist_avg = (
            ...
            if registered_timeline > 0 else 0
        )
        out_time_dist_avg = (
            round(100 * total_dict["FIR Registered Out of Timeline"] / total_dict["Complaints to FIR"], 2)
            if registered_out_timeline > 0 else 0
        )
        complaint_to_fir_dist_avg = (
            round(100 * total_dict["FIR Registered Out of Timeline"] / total_dict["Complaints to FIR"], 2)
            if total_dict["Complaints to FIR"] > 0 else 0
        )
        in_time_prov_avg = round(100 * total_dict["FIR Registered In Timeline"] / total, 2)
        out_time_prov_avg = round(100 * total_dict["FIR Registered Out of Timeline"] / total, 2)
...
def aggregate_complaints(districts, period="1m"):
    ...
    scaling_factors = {
        "1d": 0.5,   # very small numbers in 1 day
        "1w": 1.25,  # weekly smaller than monthly
        "1m": 5.0,   # baseline
        "3m": 15.0,   # bigger spread for 3 months
    }
    scale = scaling_factors.get(period, 1.0)

    return process_complaints(df, districts, scale)
